...
export const isAdmin = async (req, res, next) => {
    try {
        ...
        console.log({ token })
        if (!token) return res.status(401).json({ status: 'fail', error: 'permission denied no token in it admin' })
        // VERIFICATION TOKEN
        const decode = jwt.verify(token, process.env.JWT_SECRET);
        if (!decode._id) return res.status(401).json({ status: 'fail', error: 'permission denied incorrect token admin' })
        // CHECK IF USER EXIST
...
export const isAdminValid = async (req, res, next) => {
    try {
        ...
        // VERIFICATION TOKEN
        const decode = jwt.verify(token, process.env.JWT_SECRET);
        if (!decode._id) return res.status(401).json({ status: 'fail', message: false })
        // CHECK IF USER EXIST
        const admin = await Admin.findOne({ _id: decode._id });
